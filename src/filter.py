...
def process(info):
    ...
    # filter_depth[np.logical_not(mask)] = Z[np.logical_not(mask)]
...
def main(args):
    ...
    pred_depths = load_depths(image_list,
                              os.path.join(args.basedir, args.expname, 'nerf', 'results'))
    masks = load_masks(os.path.join(args.datadir, 'mask_l'), image_list,
                       H=pred_depths.shape[1], W=pred_depths.shape[2])

    for i in range(len(image_list)):
        pred_depths[i] = cv2.medianBlur(pred_depths[i], 3)

    N, H, W, _ = pred_rgbs.shape
    gt_rgbs = load_rgbs_np(image_list, os.path.join(args.datadir, 'l', 'images'),
                           H=H, W=W)
    ...
    info_list = [(gt_rgbs[i], pred_depths[i], masks[i], confidences[i], save_path[i])
                 for i in range(N)]
