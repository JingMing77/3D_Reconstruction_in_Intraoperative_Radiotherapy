...
def load_colmap_data(realdir, l_r=None):
    if l_r is None:
        raise ValueError(f'please input left(l) or right(r) for load ')
    camerasfile = os.path.join(realdir, l_r, 'sparse/0/cameras.bin')
    ...
    imagesfile = os.path.join(realdir, l_r, 'sparse/0/images.bin')
    ...
    n_images = len(os.listdir(os.path.join(realdir, l_r, 'images')))
    read_right = (len(names) == 2 * n_images)

    perm = []
    with open(os.path.join(realdir, 'train.txt'), 'r') as f:
        lines = f.readlines()
        if read_right:
            nums = [int(name.split('.')[0]) for name in lines]
            lines.extend([f"{i + n_images}.jpg" for i in nums])
        for line in lines:
    ...
    poses = c2w_mats[:, :3, :4].transpose([1, 2, 0])  # (3, 4, N_img)
    poses = np.concatenate([poses, np.tile(hwf[..., np.newaxis], [1, 1, poses.shape[-1]])], 1)  # pose + hwf (3, 5, N_img)

    points3dfile = os.path.join(realdir, l_r, 'sparse/0/points3D.bin')
...
def save_poses(basedir, poses, pts3d, perm, l_r=None):
    if l_r is None:
        raise ValueError(f'please input left(l) or right(r) for save poses ')
    pts_arr = []
    vis_arr = []
    for k in pts3d:
    ...
    pts_arr = np.array(pts_arr)  # (Npoints, 3): x,y,z
    vis_arr = np.array(vis_arr)  # (Npoints, Nimgs): logical
    ...
    save_arr = np.array(save_arr)  # (N_img * 17)  ( pose(3*5), bd(close_depth, inf_depth) )
...
def load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    print('Loaded image f5', imgs.shape, poses[:, -1, 0])
...
def gen_poses(basedir, factors=None):
    ...
    poses0, pts3d0, perm0 = load_colmap_data(basedir, 'l')
    # poses1, pts3d1, perm1 = load_colmap_data(basedir, 'r')
    # poses: (3, 5, N_img)  [c2w(3 * 4), hwf(3 * 1)]
    # pts3d: dict of points  with xyzrgb information
    # perm: list of train and test img_id

    save_poses(basedir, poses0, pts3d0, perm0, 'l')
    # save_poses(basedir, poses1, pts3d1, perm1, 'r')
