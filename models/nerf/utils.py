...
from models.nerf.ray_utils import get_rays, get_ray_directions, get_ndc_rays
...
def get_bbox3d_for_blenderobj(camera_transforms, H, W, near=2.0, far=6.0):
    ...
    return (
    torch.tensor(min_bound) - torch.tensor([1.0, 1.0, 1.0]), torch.tensor(max_bound) + torch.tensor([1.0, 1.0, 1.0]))
...
def get_voxel_vertices(xyz, bounding_box, resolution, log2_hashmap_size):
    ...
    if not torch.all(xyz <= box_max) or not torch.all(xyz >= box_min):
        ...
        for i in range(xyz.shape[1]):
            xyz[:, i] = torch.clamp(xyz[:, i], min=box_min[i], max=box_max[i])
