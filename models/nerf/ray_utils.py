...
def create_meshgrid(height: int, width: int, normalized_coordinates: bool = False) -> torch.Tensor:
    x_coords = torch.arange(width, dtype=torch.float32)
    y_coords = torch.arange(height, dtype=torch.float32)
    if normalized_coordinates:
        x_coords = (2 * x_coords / (width - 1)) - 1
        y_coords = (2 * y_coords / (height - 1)) - 1
    y_coords, x_coords = torch.meshgrid(y_coords, x_coords)
    meshgrid = torch.stack((x_coords, y_coords), dim=-1)
    return meshgrid
